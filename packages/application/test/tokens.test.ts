...
describe('derived Magic Link credentials', () => {
  it('derives the same high-entropy token for the same delivery request', () => {
    const key = 'SYNTHETIC_TEST_KEY_0123456789abcdef0123456789';
    const id = '11111111-1111-4111-8111-111111111111';
    const first = deriveMagicLinkToken(key, id);
    const retry = deriveMagicLinkToken(key, id);
  ...
  it('uses delivery identity and key as domain-separated inputs', () => {
    const key = 'SYNTHETIC_TEST_KEY_0123456789abcdef0123456789';
    expect(
      deriveMagicLinkToken(key, '11111111-1111-4111-8111-111111111111').raw,
    ).not.toBe(
      deriveMagicLinkToken(key, '22222222-2222-4222-8222-222222222222').raw,
    );
  });
});
