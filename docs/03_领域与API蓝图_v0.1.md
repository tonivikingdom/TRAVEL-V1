...
## 6.2 Planning Policy 与推荐策略边界（O-05 产品规则已确认）
...
- 偏好可以影响可行候选排序，但不能替代事实或约束。默认 1 个主推荐和最多 2 个可靠备选；不凑数，
  无可靠候选时明确返回没有可确认方案。
- P5C 实现的首版内部排序以最早到达为第一顺位，并在最快方案 effective total time 的 140% 内选
  同币种最低已知票价作为第二顺位；未知票价不参与最低价胜出，跨币种禁止直接比较金额。其余真实候选
  仍作为 reference，且 RouteCandidateLeg 的线路名、端点、时刻、时长与 Provider ref 不被压掉。
- 该内部排序不向前台暴露“最快/最优惠/最佳”标签。完整综合 RecommendationPolicy、正式 UI 与真实
  Provider 仍未实现。

## 6.2.1 停留与 Buffer 语义

- 系统建议停留独立持久化，不能用 `UserTimeIntent(MIN_DWELL, locked=false)` 冒充。用户主动编辑才
  建立硬的用户最低停留；删除该 Intent 即恢复建议。当前 projected dwell 始终为 departure-arrival。
- Route Query 默认提供 15 分钟 lookback 以展示真实可选路线；它只能放宽软规划基准，不能突破事实或
  用户硬边界。用户最低停留不足必须在 Preview/Adopt 中显式确认，并与 Adopt 同事务、同版本、同 receipt；
  Undo 恢复原最低值。
- 系统建议 Buffer、用户偏好 Buffer、系统最低换乘时间分别建模。用户确认可结束软偏离提醒，但不能
  让可靠系统最低换乘风险消失。
