...
| P1B1 | 持久 Job、Worker 执行框架、Magic Link 异步邮件     | 已授权；在 `feature/p1b1-jobs-mail` 实施与验证  | 只含 Job 与邮件，不进入 NotificationEvent/ObjectStorage |
| P1B2 | NotificationEvent、ObjectStorage 边界              | 未授权、未实现                                  | P1B1 验收后仍需另行授权                                 |
...
- O-00 已解决。P1B1 已获单独授权；P1B2 与 P2 之后仍受阶段闸门约束。
